...
Page({
  ...
  handeItemChange(e) {
    ...
    let { cart } = this.data;
    // 3 找到被修改的商品对象
    let index = cart.findIndex(ele => ele.goods_id === goods_id);
    // 4 选中状态取反
  ...
  handleItemAllCheck() {
    ...
    cart.forEach(ele => ele.checked = allChecked);
  ...
  async handlePay() {
    ...
    if (!address.userName) {
      await showToast({ title: "请选择您的收货地址" });
      return;
    }
    // 2 判断用户有没有选购商品
    if (totalNum === 0) {
      await showToast({ title: "请选择您的商品" });
      return;
    }
    // 3 跳转到 支付页面
    wx.navigateTo({
